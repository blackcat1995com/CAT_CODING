function content(getPages,getData){
  $(function () {
      ...
      callBack: function (now) {
        $.get(getData+'&page='+now,function(result){
          ...
          for (var i = 0; i < result.length; i++) {
            var create_dl = $('<dl></dl>');
            var _html = '<dt>'+
                          '<a href="#" target="_blank"><img src=../static/images/45/'+result[i].picture+'></a>'+
                        '</dt>'+
                        '<dd>'+
                          '<a href="#" target="_blank" class="title" title="'+result[i].title+'">'+result[i].title+'</a>'+
                          '<p class="author" title="'+result[i].ingredients+'">'+result[i].ingredients+'</p>'+
                        '</dd>';
