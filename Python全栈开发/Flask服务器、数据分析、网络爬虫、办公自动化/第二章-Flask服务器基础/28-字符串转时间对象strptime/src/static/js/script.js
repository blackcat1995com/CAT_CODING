    ...
    $.ajax({
        ...
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                jjrmodelidlist.push(data[i]._id);
                jjrmodeltimelist.push(data[i].time);
                jjrmodelztlist.push(data[i].state);
            }
...
function setRigth(year, yue, day) {
    ...
    if (bool) {
        szrqselect.options.add(new Option("上学", "1"));
    } else {
...
function aboluoSetrq() {
    ...
    if (ztid == null) {
        $.ajax({
            type: "GET",
            url: '/date',
            data: {"date": date, "state": szjjr.childNodes[1].value}, //这里用ajax可以改变状态
            async: false,
            success: function () {
        ...
        $.ajax({
            type: "GET",
            url: '/delete',
            data: {"date": date, "state": 0}, //这里用ajax可以改变状态
            async: false,
            success: function () {
                var date = getAclickDomDate();
                var datearr = date.split("-");
