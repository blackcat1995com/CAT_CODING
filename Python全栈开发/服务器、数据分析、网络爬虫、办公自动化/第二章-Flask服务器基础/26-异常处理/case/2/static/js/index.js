...
function show_detail(blog) {
    ...
    window.open(BASE_PATH + '/microblog/detail?id='+id+'&name='+name+'&photo='+photo);
}
    ...
    loading_more(function(result){
        ...
        if (result.data.length == 0) {
            ...
            for (var i = 0; i < blogs.length; i++) {
                var blog_template =
                    '<div class="kid-piece" data-id="' + blogs[i]._id + '" onclick="show_detail(this)">' +
                        '<div class="kid-piece-content" style="background-image: url(/static/images/photo/' + blogs[i].photos[0] + ');"></div>' +
                        '<div class="kid-piece-footer">' +
                            '<div class="kid-piece-photo" style="background-image: url(/static/images/photo/'+blogs[i].author.head+');"></div>' +
                            '<div class="kid-piece-name">' + blogs[i].author.username + '</div>' +
